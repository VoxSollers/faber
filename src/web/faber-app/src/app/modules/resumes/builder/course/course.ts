...
export class Course {
  ...
  protected readonly courseTitle = (item: CourseModel): string =>
    this.titles.resolve(item.id, [item.name, item.school]);

  protected getForm(item: CourseModel) {
    if (!this.formMap.has(item.id)) {
      const form = this.buildForm(item);
      form.valueChanges
        .pipe(takeUntilDestroyed(this.destroyRef))
        .subscribe(v => this.store.updateCourse({ id: item.id, ...(v as UpdateCourseRequest) }));
      this.titles.register(item.id, form, ['name', 'school']);
      this.formMap.set(item.id, form);
    }
    return this.formMap.get(item.id)!;
  }

  protected onTitleFocusIn(item: CourseModel): void {
    this.titles.onFocusIn(item.id);
  }

  protected onTitleFocusOut(item: CourseModel, event: FocusEvent): void {
    this.titles.onFocusOut(item.id, event);
  }

  private buildForm(item: CourseModel) {
