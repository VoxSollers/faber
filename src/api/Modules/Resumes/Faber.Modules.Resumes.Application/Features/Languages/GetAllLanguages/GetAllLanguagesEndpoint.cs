...
public class GetAllLanguagesEndpoint(ILogger<GetAllLanguagesEndpoint> logger)
    : Endpoint<GetAllLanguagesRequest, Results<Ok<GetAllLanguagesResponse>, NotFound>>
{
    public override void Configure()
    {
        Get("");
        Group<LanguagesSubGroup>();
        Version(1);
        Policies("ResumeOwnerPolicy");
        Options(x => x.RequireRateLimiting(RateLimitPolicies.AuthenticatedDefault));
    }

    public override async Task<Results<Ok<GetAllLanguagesResponse>, NotFound>> ExecuteAsync(
        GetAllLanguagesRequest req,
