...
public class DeleteLanguageCommandHandler(
    ...
    : ICommandHandler<DeleteLanguageCommand, ErrorOr<bool>>
{
    private const string HandlerName = nameof(DeleteLanguageCommandHandler);

    public async Task<ErrorOr<bool>> ExecuteAsync(DeleteLanguageCommand command, CancellationToken ct)
    {
        ...
        logger.LogInformation("[SUCCESS] {HandlerName} | Language {LanguageId} deleted", HandlerName, command.Id);
