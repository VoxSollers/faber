...
**Use Dapper when:** performance is critical, complex SQL (CTEs, window functions), read-heavy workloads, legacy schemas.

**Use EF Core when:** rich domain models with relationships, need change tracking, LINQ-to-SQL translation, complex object graphs.

**Repository pattern:** use only with Dapper. Do not wrap EF Core in repositories — use DbContext directly.
