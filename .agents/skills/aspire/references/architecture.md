...
| `AfterResourcesCreatedEvent` | After all resources are created (replaces `AfterResourcesCreatedAsync` from old lifecycle hooks) |
...
### DI-based subscribers (with injected services)

When the subscriber needs services from DI, register it as a singleton and resolve via `@event.Services`:

```csharp
// Extension method on IDistributedApplicationBuilder (not IServiceCollection)
public static IDistributedApplicationBuilder AddMyIntegration(
    this IDistributedApplicationBuilder builder,
    Action<MyOptions>? configure = null)
{
    builder.Services.Configure<MyOptions>(options =>
    {
        builder.Configuration.GetSection(MyOptions.SectionName).Bind(options);
        configure?.Invoke(options);
    });

    builder.Services.AddSingleton<MyHook>(); // plain class, no interface

    builder.Eventing.Subscribe<AfterResourcesCreatedEvent>(async (@event, ct) =>
    {
        var hook = @event.Services.GetRequiredService<MyHook>();
        await hook.AfterResourcesCreatedAsync(@event.Model, ct);
    });
...
Call site in `Program.cs`:

```csharp
builder.AddMyIntegration(options => { ... }); // extends IDistributedApplicationBuilder
```

> **IMPORTANT — deprecated API (Aspire 9+):** `IDistributedApplicationLifecycleHook`, `AddLifecycleHook<T>`, and extending `IServiceCollection` for AppHost-level hooks are **obsolete**. Always use `builder.Eventing.Subscribe<TEvent>` instead. Extending `IServiceCollection` loses access to `builder.Eventing` — always extend `IDistributedApplicationBuilder`.
